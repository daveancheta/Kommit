      ...
      <section id="pricing" className="mx-auto max-w-6xl px-4 py-14 scroll-mt-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-muted-foreground md:text-base">
              Keep it simple while you’re building. Adjust these tiers anytime.
            </p>
          </div>
        </div>
          ...
          {[
            {
              ...
              items: ["1 workspace", "Core chat + tasks", "Basic meetings"],
            ...
            },
            {
              name: "Team",
              price: "$9",
              blurb: "For teams who want structure and speed.",
              items: ["Unlimited projects", "Team permissions", "Notifications"],
            ...
            },
            {
              name: "Business",
              price: "$19",
              blurb: "For orgs that need more control.",
              items: ["Advanced roles", "Audit-friendly history", "Priority support"],
              cta: { label: "Contact", href: "/signup" },
              variant: "outline" as const,
            },
          ].map((p) => (
            <div
              key={p.name}
              className={[
                "rounded-3xl border border-border/60 p-6",
                p.highlight ? "bg-card/50 shadow-sm" : "bg-background/40",
              ].join(" ")}
            >
              ...
              <div className="mt-6">
                <Button asChild className="w-full" variant={p.variant}>
                  <Link href={p.cta.href}>
                    {p.cta.label} <ArrowRight className="ml-1 size-4" />
                  </Link>
                </Button>
              </div>
            </div>
