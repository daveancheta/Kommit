...
interface Groupstate {
    ...
    handleLeaveGroupValidation: (group_id: string, id: string) => Promise<void>;
...
export const UseGroupStore = create<Groupstate>((set) => ({
    ...
    handleGetGroups: async (showLoading: boolean) => {
        if (showLoading) {
            set({ isLoading: true })
        }
    ...
    handleRemoveMemberValidation: async (user_id: string, group_id: string, id) => {
        set({ isSubmitting: true })
        ...
        try {
            await fetch('/api/group/remove', {
                method: "PATCH",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ user_id, group_id, id })
            })
    ...
    handleLeaveGroupValidation: async (group_id: string, id: string) => {
        set({ isSubmitting: true })
